...
class NRIModel(nn.Module):
    ...
    def predict_states(self, states: Tensor, edges: Tensor, M: int=1, target=None, update_z=False) -> Tensor:
        """
        ...
        if (target is not None) and (update_z == True):
            # for OnlineRNNDEC
            return self.dec(states, edges, self.es, M, target, update_z)
        else:
...
class OnlineNRIModel(nn.Module):
    """Auto-encoder."""
    def __init__(self, encoder: nn.Module, decoder: nn.Module, es: Tensor, size: int):
        """
        Args:
            ...
            size: number of nodes
        """
        super(OnlineNRIModel, self).__init__()
        self.enc = encoder
        self.dec = decoder
        self.es = torch.LongTensor(es)
        self.size = size
    ...
    def forward(self, states_enc: Tensor, states_dec: Tensor, hard: bool=False, p: bool=False, M: int=1, tosym: bool=False, edges=None):
        """
        Args:
            states_enc: [batch, step_enc, node, dim], input node states for the encoder
            states_dec: [batch, step_dec, node, dim], input node states for the decoder
