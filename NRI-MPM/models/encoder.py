...
class RNNENC(GNN):
    ...
    def intra_es(self, z: Tensor, size: Tensor) -> Tensor:
        ...
        zz = z.view(E//(size-1), size-1, batch, dim) # z: (20,1,256) zz: (5,4,1,256)
        zz = zz.transpose(0, 1).contiguous().view(size-1, -1, dim) # zz: (5,4,1,256) -> (4,5,1,256) -> (4,5*1,256)
        orig = np.arange(zz.shape[1]).reshape((1, -1)).repeat(size-1, axis=0) # (1,5*1)->(4,5*1) [[01234] [01234] [01234] [01234]]
        std = np.arange(size-1).reshape((-1, 1)).repeat(E*batch//(size-1), axis=-1) # (4,1)->(4,20*1//4)=(4,5) [[00000] [11111] [22222] [33333]]
        np.random.seed()
        ...
        for i in range(size-1): # num atoms - 1 = 4
            hidden = self.intra_gru(zz[i], hidden) # zz: (4,5,256) hidden: (5,256)
            zs.append(hidden)
    ...
    def forward(self, x, es):
        ...
        # x: [batch, step, node, dim] -> [node, batch, step, dim]
        z, col, size = getattr(self, 'reduce_{}'.format(self.reducer))(x, es)
        # x: feature for each node (5,1,30,4) -> z: concatenated features for each edge (20,1,512)
        # es: [0000 1111 2222 3333 4444],[1234 0234 0134 0124 0123]
        # col: [0000 1111 2222 3333 4444], size: 5

        z = self.n2e_i(z) # (20,1,512) -> (20,1,256)
        z_skip = z

        if self.factor: # default: True
            h = self.aggregate(z, col, size) # z: (20,1,256), h: (5,1,256)
            h = self.e2n(h) # self.e2n is MLP; h: (5,1,256)
            z, _, __ = self.message(h, es) # z: (20,1,512)
            # skip connection
            z = torch.cat((z, z_skip), dim=2) # z: (20,1,768)
            z = self.n2e_o(z) # z: (20,1,256)
        else:
            z = self.e2n(z)
            z = torch.cat((z, z_skip), dim=2)
            z = self.n2e_o(z)

        E, batch, dim = z.shape # z: (20,1,256)

        if self.option == 'both': # default: both
            # NOTE: GRU is applied here
            # zs: (5,4,1,256) hs: (5,4,1,256) size=5
            zs = self.intra_es(z, size)
            hs = self.inter_es(zs, size) 
            zs = torch.cat([zs, hs], dim=-1) # zs: (5,4,1,512)
            z = zs.view(-1, batch, dim * 2) # z: (20,1,512)
            ...
            z = z.view(-1, batch, dim)
        z = self.fc_out(z) # final z: (20,1,2)
        return z
