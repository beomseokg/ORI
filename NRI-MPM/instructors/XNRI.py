...
class XNRIIns(Instructor):
    ...
    def single_batch_train(self):
        ...
        for batch_idx, (adj, states) in enumerate(data):
            ...
            for i in range(num_iters):
                ...
                if self.args.online == True:
                    loss, mse, mse_list, acc = self.online_train_nri(temp_data_aug, adj, i + batch_idx * num_iters)
    ...
    def online_train_nri(self, states: Tensor, adj: Tensor, batch_idx: int) -> Tensor:
        """
        ...
        if batch_idx % 1 == 0 and ("motion" not in self.cmd.dyn):
