...
class MLP(nn.Module):
    ...
    def __init__(self, n_in, n_hid, n_out, do_prob=0.):
        ...
        #print("[MLP] n_in, n_hid, n_out: ", n_in, n_hid, n_out)
        #input("here")
...
class MLPEncoder(nn.Module):
    ...
    def forward(self, inputs, rel_rec, rel_send):
        ...
        #print("[MLPEncoder] out: ", self.fc_out(x).shape) # (1, num_edges=20, num_edge_types=2)
        #input("here")
...
class MLPDecoder(nn.Module):
    ...
    def __init__(self, n_in_node, edge_types, msg_hid, msg_out, n_hid,
        ...
        self.out_fc1 = nn.Linear(n_in_node + msg_out, n_hid) # default -> comment
        #self.out_fc1 = nn.Linear(msg_out, n_hid, bias=False) # default bias=True -> modified: bias=False
        self.out_fc2 = nn.Linear(n_hid, n_hid)#, bias=False) # default bias=True -> modified: bias=False
        self.out_fc3 = nn.Linear(n_hid, n_in_node)#, bias=False) # default bias=True -> modified: bias=False
    ...
    def single_step_forward(self, single_timestep_inputs, rel_rec, rel_send,
        ...
        # Skip connection
        aug_inputs = torch.cat([single_timestep_inputs, agg_msgs], dim=-1) # default -> comment
        #aug_inputs = agg_msgs # use without input information
    ...
    def forward(self, inputs, rel_type, rel_rec, rel_send, pred_steps=1, test=False):
        # NOTE: Assumes that we have the same graph across all samples.
        ...
        if test == True:
            ...
            last_pred = inputs[:,0:20,:,:]
            curr_rel_type = rel_type[:,0:20,:,:]
            ...
            pred_all = torch.cat(preds, dim=1).permute(0,2,1,3).contiguous() # (1,5,200,4)
            return pred_all


class OnlineMLPDecoder(nn.Module):
    """MLP decoder module."""

    def __init__(self, n_in_node, edge_types, msg_hid, msg_out, n_hid,
                 do_prob=0., skip_first=False):
        super(OnlineMLPDecoder, self).__init__()
        self.msg_fc1 = nn.ModuleList(
            [nn.Linear(2 * n_in_node, msg_hid) for _ in range(edge_types)])
        self.msg_fc2 = nn.ModuleList(
            [nn.Linear(msg_hid, msg_out) for _ in range(edge_types)])
...
class RNNDecoder(nn.Module):
    ...
    def forward(self, data, rel_type, rel_rec, rel_send, pred_steps=1,
        ...
        #print("preds: ", preds.shape) # (num_batch=1, num_timesteps=59, num_atoms=5, num_features=4)
        #input("here")

        return preds.transpose(1, 2).contiguous()


class OnlineRNNDecoder(nn.Module):
    """Recurrent decoder module."""

    def __init__(self, n_in_node, edge_types, n_hid,
                 do_prob=0., skip_first=False, n_atoms=5):
        super(OnlineRNNDecoder, self).__init__()
        self.msg_fc1 = nn.ModuleList(
            [nn.Linear(2 * n_hid, n_hid) for _ in range(edge_types)])
        self.msg_fc2 = nn.ModuleList(
            [nn.Linear(n_hid, n_hid) for _ in range(edge_types)])
        ...
        self.hidden_r = nn.Linear(n_hid, n_hid, bias=False) # default bias: False
        self.hidden_i = nn.Linear(n_hid, n_hid, bias=False)
        self.hidden_h = nn.Linear(n_hid, n_hid, bias=False)
        ...
        self.out_fc1 = nn.Linear(n_hid, n_hid) # default bias: True
        self.out_fc2 = nn.Linear(n_hid, n_hid)
        self.out_fc3 = nn.Linear(n_hid, n_in_node)
    ...
    def single_step_forward(self, inputs, rel_rec, rel_send,
        ...
        pre_msg = torch.cat([senders, receivers], dim=-1)
        # pre_msg (1,20,512) 
        # pre_msg[:,:4,:] 2->1, 3->1, 4->1, 5->1
        # pre_msg[:,4:8,:] 1->2, 3->2, 4->2, 5->2 etc
        ...
        """
        NOTE: we use skip_first in online learning
        """
    ...
    def forward(self, data, rel_type, rel_rec, rel_send, pred_steps=1,
                burn_in=False, burn_in_steps=1, dynamic_graph=False,
                encoder=None, temp=None, test=False, target=None):
        ...
        if burn_in_steps != 0:
            for step in range(0, inputs.size(1) - 1):
                ...
                if test == False:
                    if step < 20:
                        ins = inputs[:, step, :, :] # (num_batch=1, num_atoms=5, num_features=4)

                        temp_pred = []
                        for i in range(10):
                            if i == 0:
                                pred, hidden = self.single_step_forward(ins, rel_rec, rel_send, rel_type, hidden) # output (5,4)
                                temp_hidden = hidden
                            else:
                                pred, temp_hidden = self.single_step_forward(pred_all[-1], rel_rec, rel_send, rel_type, temp_hidden) # output (5,4)
                            temp_pred.append(pred)
                            pred_all.append(pred)
                        temp_pred = torch.stack(temp_pred, dim=1)
                    ...
                    if step < burn_in_steps:
                        ...
                        pred, hidden = self.single_step_forward(pred_all[-1], rel_rec, rel_send, rel_type, hidden) # output (5,4)
                        pred_all.append(pred)

        elif burn_in_steps == 0:
            inputs_m = inputs[:,0::pred_steps,:,:][0]
            if burn_in_steps == 1:
                hidden = Variable(
                    torch.zeros(inputs_m.size(0), inputs_m.size(1), self.msg_out_shape)).cuda()
            for step in range(pred_steps):
                inputs_m, hidden = self.single_step_forward(inputs_m, rel_rec, rel_send, rel_type, hidden) # output (5,4)
                pred_all.append(inputs_m)
        preds = torch.stack(pred_all, dim=1)

        if burn_in_steps == 0:
            preds_hat = Variable(torch.zeros_like(inputs)).cuda()
            for i in range(pred_steps):
                preds_hat[:,i::pred_steps,:,:] = preds[:,i]
            return preds_hat[:,:(inputs.shape[1]-1)].transpose(1, 2).contiguous(), rel_type

        return preds.transpose(1, 2).contiguous(), rel_type
